...
def _fill_tx_nulls_same_session(target,fallback,session_date,audit):
    fb_date=fallback.get("session_trade_date");dst_date=target.get("session_trade_date") or session_date
    if fb_date!=session_date or dst_date!=session_date:
        audit.append({"section":"taiwan_futures","status":"SESSION_DATE_MISMATCH_NO_OVERRIDE","canonical_session_date":dst_date,"fallback_session_date":fb_date,"wanted":session_date});return
    filled=[];differences=[]
    compare_keys={"night_close","change_points","change_pct","night_high","night_low","volume","total_oi","foreign_long_oi","foreign_short_oi","foreign_net_oi","foreign_net_change"}
    for k,v in fallback.items():
        if k in {"contract","session_trade_date"}:
            if not target.get(k) and v is not None:target[k]=v;filled.append(k)
            continue
        if target.get(k) is None and v is not None:
            target[k]=v;filled.append(k)
        elif k in compare_keys and target.get(k) is not None and v is not None and target.get(k)!=v:
            differences.append({"field":k,"canonical":target.get(k),"fallback":v})
    audit.append({"section":"taiwan_futures","status":"MATCH_SAME_SESSION_NULL_FILL_ONLY","session_trade_date":session_date,"filled_fields":filled,"differences_no_override":differences})
...
def main():
    ...
    tx_existing=report.get("taiwan_futures") or {};session_date=tx_existing.get("session_trade_date") or date_text;fetched_tx=fetch_night_session(session_date,validation);_fill_tx_nulls_same_session(tx_existing,fetched_tx,session_date,validation);report["taiwan_futures"]=tx_existing;fill_missing(report,report.get("market_trade_date") or date_text,validation)
